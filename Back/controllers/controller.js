...
const home = async (req, res) => {
  ...
  try{
   ...
   for(var i=0;i<response.results.length;i++){
    if(response.results[i]?.properties?.ID?.people[0]?.id===undefined) {continue;}
    // console.log(response.results[i]?.properties?.ID?.people[0]?.id);
...
const mfd = async (req, res) => {
  ...
  const cookie = req.cookies.token;
  if (!cookie) {
    return res.status(400).json({ message: "Session Expired3" });
  }
  try{
    const data = jwt.verify(cookie, process.env.SECRET);
...
const forgotpassword = async (req, res) => {
  ...
  if (response.results.length === 0) {
    ...
    var mailOptions = {
      ...
      text: 'Link is valid for 15 minutes\n\nhttp://localhost:5173/createpassword?token=' + token
    };
